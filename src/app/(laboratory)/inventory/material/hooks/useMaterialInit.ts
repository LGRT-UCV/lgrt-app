...
import {
  getMaterialTypes,
  getMeasurements,
  getSGAClassification,
  getStoragePlaces,
} from "../../utils";

type TRequestMaterialInit =
  | "measurements"
  | "materialType"
  | "sgaClassification"
  | "storagePlace";
...
export default function useMaterialInit(request?: Array<TRequestMaterialInit>) {
  ...
  const { data: measurementList = [], isLoading: isMeasurementsLoading } =
    useQuery({
  ...
  const { data: materialTypeList = [], isLoading: isMaterialTypeLoading } =
    useQuery({
  ...
  const { data: sgaClassification = [], isLoading: isSgaClassLoading } =
    useQuery({
  ...
  const { data: storagePlace = [], isLoading: isStoragePlaceLoading } =
    useQuery({
